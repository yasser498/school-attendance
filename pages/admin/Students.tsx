...
const Students: React.FC = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      // Reuse the existing batch sync logic, passing empty add/update arrays and the selected IDs for deletion
      await syncStudentsBatch([], [], selectedIds);
      setSelectedIds([]);
      // No need to re-fetch immediately if optimistic update works, 
      // but syncing in background is good practice.
    } catch (error) {
    ...
    try {
        ...
        data.forEach((row: any, index: number) => {
            ...
            if (name && studentIdRaw) {
                ...
                if (!name && !studentIdRaw) return; // Skip completely empty rows
        ...
        const confirmMsg = `تقرير المعالجة:\n➕ طلاب جدد: ${toAdd.length}\n🔄 تحديث بيانات: ${toUpdate.length}\n⚠️ صفوف تم تجاهلها: ${errors.length}\n\nهل تريد اعتماد البيانات؟`;
    ...
    <div className="space-y-6 animate-fade-in relative">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
            ...
            <p className="text-slate-500 text-sm mt-1">رفع وإدارة بيانات الطلاب (تخزين سحابي)</p>
        </div>
