...
import { FileText, CheckCircle, XCircle, Search, Loader2, Paperclip, Eye, X, Download, School, Trash2 } from 'lucide-react';
import { getRequests, updateRequestStatus, deleteAttachmentFile, removeRequestAttachmentRef } from '../../services/storage';
import { ExcuseRequest, RequestStatus, StaffUser } from '../../types';
...
const StaffRequests: React.FC = () => {
  ...
  // Only show requests for assigned classes
  const myClassRequests = useMemo(() => {
      if (!currentUser?.assignments) return [];
      return requests.filter(r => {
          // Robust check: Ensure r.grade and r.className exist (filled by storage.ts)
          // Then check if it matches any of the teacher's assignments
          return currentUser.assignments.some(
              a => a.grade === r.grade && a.className === r.className
          );
      });
  }, [requests, currentUser]);

  const filteredRequests = useMemo(() => {
      return myClassRequests.filter(r => {
  ...
  }, [myClassRequests, filter, search]);

  const stats = useMemo(() => {
      return {
          total: myClassRequests.length,
          pending: myClassRequests.filter(r => r.status === 'PENDING').length,
          approved: myClassRequests.filter(r => r.status === 'APPROVED').length,
          rejected: myClassRequests.filter(r => r.status === 'REJECTED').length
      };
  }, [myClassRequests]);
    ...
    <div className="space-y-6 animate-fade-in pb-20 relative">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
          <h1 className="text-2xl font-bold text-blue-900 flex items-center gap-2 mb-6">
                <FileText className="text-amber-500" /> طلبات الأعذار (فصولي)
          </h1>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div onClick={() => setFilter(RequestStatus.PENDING)} className={`p-4 rounded-2xl border cursor-pointer transition-all ${filter === RequestStatus.PENDING ? 'bg-amber-50 border-amber-300' : 'bg-white border-slate-100 hover:border-amber-200'}`}>
                  <p className="text-xs text-amber-600 font-bold uppercase">جديد</p>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
          <div className="relative">
              ...
              <input 
                ...
                placeholder="بحث..." 
                className="w-full pr-10 pl-4 py-3 bg-slate-50 border-none rounded-xl outline-none font-bold text-slate-700"
              />
          </div>
      </div>
      ...
      {loading ? (
          ...
          <div className="py-20 text-center text-slate-400 bg-white rounded-3xl border border-dashed border-slate-200">
              ...
              <p>لا يوجد طلبات مطابقة في الفصول المسندة إليك</p>
          </div>
      ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredRequests.map(req => (
                  <div key={req.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col">
                      <div className="p-4 border-b border-slate-50 bg-slate-50/50 flex justify-between items-start">
                          <div className="flex items-center gap-3">
                              <div className="w-12 h-12 rounded-full bg-white border border-slate-200 text-blue-700 flex items-center justify-center font-bold text-lg shadow-sm">
                                  {(req.studentName || '?').charAt(0)}
                              </div>
                              <div>
                                  <h3 className="font-bold text-slate-800 text-base leading-snug">{req.studentName || 'اسم غير متوفر'}</h3>
                          ...
                          <span className={`px-2 py-1 rounded-lg text-[10px] font-bold ${statusStyles[req.status]}`}>
                      ...
                      <div className="p-4 flex-1">
                          ...
                          {req.details && <p className="text-xs text-slate-500 bg-slate-50 p-2 rounded-lg leading-relaxed mt-2 border border-slate-100">{req.details}</p>}
                      ...
                      <div className="p-3 bg-slate-50 border-t border-slate-100 flex gap-2">
                          <button 
                            ...
                            className="flex-1 bg-emerald-600 text-white py-2.5 rounded-xl text-xs font-bold hover:bg-emerald-700 flex items-center justify-center gap-1 shadow-sm disabled:opacity-50"
                            disabled={req.status !== RequestStatus.PENDING}
                          >
                              <CheckCircle size={14}/> قبول
                          </button>
                          <button 
                            onClick={() => handleStatusUpdate(req.id, RequestStatus.REJECTED)} 
                            className="flex-1 bg-white border border-red-200 text-red-600 py-2.5 rounded-xl text-xs font-bold hover:bg-red-50 flex items-center justify-center gap-1 disabled:opacity-50"
                            disabled={req.status !== RequestStatus.PENDING}
                          >
                              <XCircle size={14}/> رفض
                          </button>
